...
x_smiles = test_data['canonical_smiles'].values.astype('str')
y = test_data['standard_value'].values.astype('float32')
...
class GCN(torch.nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.lin_edge = torch.nn.Linear(10, 1)

    def forward(self, x, edge_index, edge_attr):
        x = F.relu(self.conv1(x, edge_index))
        x = self.conv2(x, edge_index)
        edge_scores = self.lin_edge(edge_attr)
...
for tensor in data_list:
    output = gcn(tensor.x, tensor.edge_index, tensor.edge_attr)
    print(output)
